...
def generate_image(input_text: str) -> str:
    def get_photo_prompt(input_text, scene) -> str:
        style_keywords = [
            "best quality",
            "ultra high res",
            "(photorealistic:1.4)",
    ...
    def get_ghibli_anime_prompt(input_text, scene) -> str:
        style_keywords = [
            "Studio Ghibli style",
    ...
    def get_modern_anime_prompt(input_text, scene) -> str:
        # 現代風アニメのスタイルキーワードリスト
    ...
    def get_kids_anime_prompt(input_text, scene) -> str:
    ...
    def get_manga_prompt(input_text, scene) -> str:
...
def main():
    input_text = """
照山紅葉[てるやまもみじ]はフロアのテーブルを磨いていた。あと 2 時間もすれば、今夜もまた賑やかな夜になるだろう。
この店がなんとかやっていけているのは、料理がうまいわけでも、紅葉の器量がいいわけでもない。
在りし日の旦那がいた頃から、常連のみんなが支えてくれているからだ。紅葉はそう認識している。
それでも、あとどのくらい、こんな日が続けられるのだろうか。
